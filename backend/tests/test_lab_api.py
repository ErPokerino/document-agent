...
def test_a_reviewed_run_can_be_promoted_to_ground_truth(api) -> None:
    ...
    promoted = api.post("/api/datasets/invoices/documents/from-run", json={"run_id": run_id})

    assert promoted.status_code == 201
    assert promoted.json()["labelled"] is True
...
def test_promoting_an_unknown_run_is_a_404(api) -> None:
    ...
    assert api.post("/api/datasets/invoices/documents/from-run", json={"run_id": 999}).status_code == 404
